...
\section{Pruebas de Usabilidad y Validación de Producto}

Con el objetivo de garantizar no solo la corrección técnica, sino también la adecuación del producto al mercado y su valor didáctico, se diseñó una estrategia de validación cualitativa (\textit{User Testing}) con cuatro perfiles estratégicos.

Esta fase ha sido fundamental para elevar la calidad final del proyecto, permitiendo refinar la experiencia de usuario (UX) y confirmar las hipótesis pedagógicas planteadas en los objetivos del TFG.

\subsection{Perfiles de Validación}
Se seleccionó un panel de usuarios complementarios para auditar todas las dimensiones de la aplicación:

\begin{itemize}
    \item \textbf{Perfil A (Experto Institucional):} Embajador Tecnológico e inversor activo. \textit{Objetivo:} Auditar la robustez de los datos y la seriedad de la propuesta.
    \item \textbf{Perfil B (Usuario Novel):} Usuario sin conocimientos previos. \textit{Objetivo:} Evaluar la curva de aprendizaje y la accesibilidad.
    \item \textbf{Perfil C (Especialista UI/UX):} Diseñador web. \textit{Objetivo:} Analizar la jerarquía visual y la usabilidad en dispositivos móviles.
    \item \textbf{Perfil D (Inversor Tradicional):} Conocimientos financieros sólidos con perfil digital básico. \textit{Objetivo:} Validar la utilidad real de la herramienta para combatir malos hábitos de inversión.
\end{itemize}

\subsection{Análisis de Resultados}

Los resultados de las sesiones de prueba fueron altamente satisfactorios, validando la propuesta de valor y permitiendo un pulido final de la interfaz:

\begin{enumerate}
    \item \textbf{Validación Pedagógica (Éxito del Modelo):} El \textbf{Perfil Inversor (D)} confirmó la gran utilidad de la aplicación para resolver un problema real del mercado: el exceso de operativa (\textit{overtrading}) en inversores novatos. Valoró muy positivamente que el simulador penalice la especulación y premie la estrategia a largo plazo, validando así el núcleo didáctico del TFG.
    
    \item \textbf{Profesionalización de la Interfaz:} Gracias al feedback del \textbf{Perfil Experto (A)} y del \textbf{Diseñador (C)}, se elevó el estándar visual de la aplicación. Se optimizó el \textit{hero section} para maximizar el espacio útil y se refinó la terminología para eliminar anglicismos innecesarios, logrando un acabado más limpio y profesional.
    
    \item \textbf{Eliminación de Barreras de Entrada:} El \textbf{Perfil Novel (B)} permitió detectar y solucionar fricciones iniciales en la terminología financiera (Ticker, CAGR), haciendo la herramienta accesible para el público general.
\end{enumerate}

\subsection{Evolución del Producto (Mejoras Implementadas)}

A raíz de este ciclo de validación, la versión final entregada incorpora las siguientes optimizaciones, cerrando el ciclo de mejora continua:
...
\hline
\textbf{Área de Mejora} & \textbf{Observación del Usuario} & \textbf{Solución Implementada} \\ \hline
\textbf{Accesibilidad Cognitiva} & \textit{"El concepto de 'Ticker' puede ser confuso para no iniciados."} (Perfil Novel) & Integración de asistencia contextual y sugerencias de búsqueda en tiempo real. \\ \hline
\textbf{Experiencia Móvil} & \textit{"La pantalla de inicio puede aprovechar mejor el espacio vertical."} (Perfil Diseño) & Rediseño del \textit{layout} principal para priorizar las llamadas a la acción (\textit{CTA}) críticas. \\ \hline
\textbf{Claridad de Propuesta} & \textit{"Debe quedar claro que es un simulador profesional, no un juego."} (Perfil Experto) & Refinamiento del \textit{copywriting} en la \textit{landing page} para enfatizar el "Análisis Fundamental". \\ \hline
\textbf{Refuerzo Positivo} & \textit{"Es vital enseñar que la inactividad a veces es la mejor estrategia."} (Perfil Inversor) & Ajuste en el sistema de puntuación del "Modo Carrera" para premiar la consistencia a largo plazo. \\ \hline
\end{tabular}
\caption{Matriz de mejoras implementadas tras la validación con usuarios.}
\label{tab:usabilidad_feedback}
\end{table}

\subsection{Pruebas de Carga y Concurrencia (Accesibilidad del Servicio)}

Para responder a la necesidad de estimar la capacidad real del servicio y determinar cuántos usuarios pueden acceder simultáneamente sin degradar la experiencia ("accesibilidad del servidor"), se realizaron pruebas de carga utilizando la herramienta \textbf{Locust} (ver archivo \texttt{locustfile.py} en el repositorio).

Se configuró un escenario de estrés contra el entorno de producción en Render (Plan Gratuito) simulando el comportamiento típico de un usuario: navegar por la portada, consultar una empresa y generar una simulación.

\begin{itemize}
    \item \textbf{Umbral de Concurrencia:} El sistema mantiene tiempos de respuesta estables (<500ms) con hasta \textbf{50 usuarios concurrentes} realizando peticiones por segundo.
    \item \textbf{Punto de Saturación:} A partir de 60-70 usuarios simultáneos, el servidor Gunicorn (limitado a 1 \textit{worker} por la memoria de 512MB) comienza a encolar peticiones, elevando la latencia por encima de los 2 segundos.
\end{itemize}

Se concluye que la arquitectura actual garantiza la accesibilidad para un uso académico o de grupo pequeño, cumpliendo los requisitos iniciales, aunque para un despliegue masivo sería necesario escalar verticalmente la infraestructura.
