...
La aplicación desarrollada en este TFG se diferencia de estas soluciones en varios aspectos. En primer lugar, integra un \textit{modo carrera} que simula el paso del tiempo y la aparición de eventos macroeconómicos predefinidos, de manera que el usuario debe adaptar su asignación de capital turno a turno. En segundo lugar, el foco se sitúa en la inversión a largo plazo y en el análisis fundamental de empresas, utilizando métricas básicas y estrategias como el \textit{Dollar Cost Averaging}, en lugar de fomentar la operativa especulativa de muy corto plazo. Además, la interfaz y las mecánicas se diseñan con un enfoque gamificado y didáctico, de forma que el usuario pueda aprender mientras experimenta con escenarios realistas sin riesgo económico directo. Finalmente, el sistema incorpora un mecanismo de \textit{feedback} inmediato poco habitual en los brókers reales: una evaluación cualitativa de la cartera. Al finalizar una sesión, el usuario recibe una puntuación basada no solo en la rentabilidad absoluta, sino también en la calidad de la gestión del riesgo, lo que refuerza el aprendizaje del equilibrio entre rentabilidad y riesgo.
...
\subsection{Integración Continua \texorpdfstring{(CI)}{(CI)}: GitHub Actions}
...
\subsection{Despliegue y alojamiento \texorpdfstring{(CD)}{(CD)}: Render}

La puesta en producción de la aplicación se realiza en la plataforma en la nube \textbf{Render} \cite{render}, que ofrece un modelo de tipo PaaS (\textit{Platform as a Service}). Render se eligió por su integración directa con GitHub, por la sencillez de configuración y por resultar adecuada para el alcance académico del proyecto. Una vez definido el servicio, la plataforma detecta automáticamente los cambios en la rama configurada, construye la aplicación y la despliega, implementando así un flujo básico de \textbf{despliegue continuo}.
...
\subsection{Frontend: Jinja2 y Chart.js}

Para la interfaz de usuario, se ha optado por una arquitectura de renderizado en servidor (SSR) utilizando el motor de plantillas \textbf{Jinja2} \cite{jinja2}, integrado nativamente en Flask. Esta decisión evita la complejidad técnica de gestionar una \textit{Single Page Application} (SPA) con \textit{frameworks} como React o Vue, permitiendo un desarrollo más ágil y centrado en la funcionalidad financiera.

En cuanto a la visualización de datos, se ha seleccionado la librería \textbf{Chart.js} \cite{chartjs}. A diferencia de alternativas más complejas como D3.js, Chart.js permite implementar gráficos financieros interactivos y adaptables a dispositivos móviles (\textit{responsive}) con una curva de aprendizaje mucho más suave, lo que se alinea con el objetivo de mantener un código mantenible y ligero.