...
\section{Metodología asistida con Cocoliso/OpenClaw}

Durante la evolución del proyecto se empleó un agente de asistencia al desarrollo, integrado mediante OpenClaw y utilizado bajo supervisión humana continua. Esta metodología no sustituyó la autoría técnica del trabajo, sino que sirvió como apoyo para organizar iteraciones, documentar cambios, revisar consistencia visual, proponer diagnósticos y acelerar tareas repetitivas de validación.

El flujo de trabajo se estructuró de forma iterativa sobre ramas específicas, con revisión humana de resultados y validación real sobre Render antes de consolidar cambios relevantes. Entre las ramas utilizadas destacaron la rama principal \texttt{main}, la rama de vista previa \texttt{bot/render-preview}, la rama de trabajo del rediseño \texttt{feature/frontend-redesign-v2} y la rama de congelación previa \texttt{stable/pre-redesign}.

Esta metodología permitió:
\begin{itemize}
    \item trabajar con cambios pequeños y trazables;
    \item documentar cada iteración importante;
    \item validar visualmente sobre entorno desplegado;
    \item separar trabajo experimental de la versión estable;
    \item corregir incidencias reales sin interrumpir el desarrollo completo del sistema.
\end{itemize}

Desde una perspectiva académica, el uso del agente puede interpretarse como una forma de desarrollo asistido y supervisado, análoga a otras herramientas modernas de apoyo a la ingeniería del software, siempre subordinada a la decisión y validación humana.
