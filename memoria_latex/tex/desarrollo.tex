...
\subsection{Especificación de Casos de Uso Extendidos}

Para detallar la lógica de negocio, se presenta la especificación completa del caso de uso más crítico del sistema: la gestión del avance de turno en el simulador.

\begin{table}[h]
\centering
\begin{tabular}{|p{0.3\textwidth}|p{0.65\textwidth}|}
\hline
\textbf{Caso de Uso} & \textbf{CU-03: Avanzar Turno} \\ \hline
\textbf{Actor} & Usuario Inversor \\ \hline
\textbf{Precondiciones} & El usuario tiene una sesión de juego activa y no ha agotado el número máximo de turnos. \\ \hline
\textbf{Flujo Principal} & 
1. El usuario ajusta la distribución de su cartera (compras/ventas). \newline
2. El usuario pulsa "Finalizar Turno". \newline
3. El sistema valida que la suma de pesos de la cartera no exceda el 100\%. \newline
4. El sistema descarga los datos históricos del periodo correspondiente. \newline
5. El sistema calcula el rendimiento de la cartera y aplica eventos aleatorios (si proceden). \newline
6. El sistema actualiza el capital acumulado y devuelve el resumen del turno. \\ \hline
\textbf{Postcondiciones} & La fecha de la simulación avanza al siguiente periodo y se actualiza el valor patrimonial del usuario. \\ \hline
\textbf{Excepciones} & Si no hay datos históricos para un activo en el periodo seleccionado, el sistema notifica el error y no permite avanzar hasta corregir la cartera. \\ \hline
\end{tabular}
\caption{Especificación extendida del caso de uso "Avanzar Turno".}
\label{tab:cu_avanzar}
\end{table}

\section{Diseño del Sistema}

Para satisfacer los requisitos funcionales y no funcionales, se ha optado por una arquitectura web basada en el patrón \textbf{Modelo-Vista-Controlador (MVC)} \cite{fowler_mvc}, adaptada al \textit{microframework} Flask.
...
A diferencia de las aplicaciones tradicionales basadas en SQL, este sistema utiliza una estructura de datos documental (JSON) para gestionar el estado de la sesión de juego en memoria. Esto permite una mayor flexibilidad al evolucionar la lógica del juego. A continuación se describe la estructura principal del objeto \texttt{Session}:

\begin{itemize}
    \item \texttt{session\_id}: Identificador único de la partida.
    \item \texttt{config}: Parámetros de dificultad (inversión inicial, volatilidad de eventos).
    \item \texttt{turns}: Lista de objetos que representan el historial de cada turno (fecha, asignación de activos, rendimiento obtenido, eventos sufridos).
    \item \texttt{portfolio}: Estado actual de la cartera (tickers y pesos).
\end{itemize}
...
Para garantizar la consistencia de los datos históricos, se ha implementado una función robusta de extracción de datos que gestiona la conexión con la API de Yahoo Finance. El siguiente fragmento muestra cómo se procesan y normalizan los precios ajustados, incluyendo la optimización de memoria implementada:
...
def _build_normalized_series_map(tickers, start, end):
    ...
    for ticker in non_cash:
        ...
        # Limpieza explicita de memoria tras descarga
        gc.collect() 
